...
@app.on_message(filters.media_group & filters.channel)
async def echo3(client, message_app):
...
{f'<a href="{channel_link}/{i.id}">@</a>' if '+' not in channel_link else ""}"""
    ...
    logger.info(
        f"[{datetime.now()}] Send message mediagroup. Users: [{actual_all_user}]\n Message entity:\n {str(message_app)}"
    )
    for user in actual_all_user:
        try:
            ...
            logger.error(f"[{datetime.now()}] Error: {er}")
    session = Session()
    logger.info(f"[{datetime.now()}] Caption text in final[{new_caption_for_base}]\n")
    for user in actual_all_user:
        new_message = Message(
