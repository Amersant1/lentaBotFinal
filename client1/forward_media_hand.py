...
@app.on_message(filters.forwarded & filters.media & filters.channel)
async def echo2(client, message_app):
    ...
    logger.info(
        f"[{datetime.now()}] Send message in media with forward. Users: [{actual_all_user}]\n Message entity:\n {str(message_app)}"
    )
...
{f'<a href="{channel_link}/{message_app.id}">@</a>' if '+' not in channel_link else ""}"""
    if message_app.photo is not None:
        ...
        try:
            ...
            for user in actual_all_user:
                if message_app.chat.username is None:
                    await bot.send_photo(user, InputFile(path), caption=caption_text)
                else:
                    await bot.send_photo(user, InputFile(path), caption=caption_text)
            logger.error(f"[{datetime.now()}] Error: {er}")
    if message_app.video is not None:
        try:
            ...
            mess_for_bot = None
            for user in actual_all_user:
                if mess_for_bot is None:
                    path = await client.download_media(message_app)
                    if message_app.chat.username is None:
                        mess_for_bot = await bot.send_video(
                            user,
                            InputFile(path),
                            caption=caption_text,
                            supports_streaming=True,
            ...
            for user in actual_all_user:
                if message_app.chat.username is None:
                    await bot.send_document(
                        user,
                        InputFile(path),
                        caption=f"{message_app.chat.title}\nForwarded from {message_app.forward_from_chat.title}\n{message_app.caption if message_app.caption is not None else ''}\n@",
                    )
                else:
                    await bot.send_document(
                        user,
        ...
        try:
            ...
            for user in actual_all_user:
                try:
                    if message_app.chat.username is None:
                        await bot.send_voice(
                            user, InputFile(path), caption=caption_text
                        )
                    else:
                        await bot.send_voice(
                            user, InputFile(path), caption=caption_text
